    ...
    return Column(
      ...
      children: [
        ...
        if (d.bindingIds.isNotEmpty)
          _TempSparkline(config: service.config, globalId: d.bindingIds.first),
...
// ── Sparkline del historial de temperatura (últimos 7 días) ─────────────────

/// Mini-gráfico del historial de temperatura ambiente (currentTemp) del
/// termostato, espejando el del dashboard: línea con gradiente naranja sobre
/// una cápsula hundida, con rótulo + rango mín–máx. Trae las lecturas vía
/// GET /api/events (filtra currentTemp, últimos 7 días) y promedia por buckets
/// para mostrar TENDENCIA y no el ruido de cada lectura. Se pide una vez (en
/// initState); si no hay datos suficientes no se muestra nada.
class _TempSparkline extends StatefulWidget {
  const _TempSparkline({required this.config, required this.globalId});

  final ServerConfig config;
  final String globalId;

  @override
  State<_TempSparkline> createState() => _TempSparklineState();
}

class _TempSparklineState extends State<_TempSparkline> {
    ...
    try {
      ...
      for (final e in page.items) {
        ...
        if (v is num) {
          ...
          if (t >= cutoff) pts.add((t: t, v: v.toDouble()));
        }
      }
